...
exports.get_store_providers = function (request_data, response_data) {
    utils.check_unique_details(request_data, [], function (response) {
        if (response.success) {
            ...
            City.findOne({_id: response.franchise.city_id}).then((city) => {
                var provider_query = {};
                var distance = setting_detail.default_search_radius / UNIT.DEGREE_TO_KM;
                    provider_query = {
                        location: {$near:  response.franchise.location, $maxDistance: distance},
                        //is_online: true,
                        is_approved: true,
                        //is_active_for_job: true,
                        city_id: response.franchise.city_id
                    }

                if(setting_detail.is_upload_provider_documents){
                    provider_query['is_document_uploaded'] = true;
                }
                if(response.franchise.is_store_can_add_provider){
                    provider_query['provider_type_id'] = response.franchise._id;
                }
    ...
    utils.check_unique_details(request_data, [], function (response) {
        if (response.success) {
			...
			var store_detail = response.franchise;
                        ...
                        var store_id_condition = {"$match": {'order_detail.store_id': {$in:store_detail.store_ids}}};
    ...
    utils.check_unique_details(request_data, [], function (response) {
        if (response.success) {
            ...
            var store_detail = response.franchise;
                        ...
                        var condition = {"$match": {'store_id': {$in:store_detail.store_ids}}};
                        var condition1 = {"$match": {'request_status': {$eq: ORDER_STATE.NO_DELIVERY_MAN_FOUND}}};
                        var store_id_condition = {"$match": {'store_id': {$in:store_detail.store_ids}}};
        ...
        if (response.success) {
            ...
            var store_detail = response.franchise;
                        Country.findOne({_id: store_detail.country_id}).then((country_detail) => {
                            ...
                            var limit = {};
                            limit["$limit"] = number_of_rec;
                            var store_condition = {"$match": {'store_id': {$in:store_detail.store_ids}}};
