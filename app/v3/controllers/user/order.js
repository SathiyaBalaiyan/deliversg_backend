...
// let users_documents = [];
//                 ReorderItems.forEach((ReorderItems, index) => {
//                     users_documents.push({
//                             cart_unique_token: "",
//                             reorder: ReorderItems.reorder,
//                             user_id: ReorderItems.user_id,
//                             user_type: ReorderItems.user_type,
//                             user_type_id: ReorderItems.user_type_id,
//                             store_id: ReorderItems.store_id,
//                             order_payment_id: null,
//                             order_id: null,
//                             city_id: ReorderItems.city_id,
//                             total_item_tax: ReorderItems.total_item_tax,
//                             delivery_type: ReorderItems.delivery_type,
//                             language: ReorderItems.language,     
//                             total_item_count: ReorderItems.total_item_count,
//                             pickup_addresses: ReorderItems.pickup_addresses,
//                             destination_addresses: ReorderItems.destination_addresses,
//                             order_details: ReorderItems.order_details,
//                             total_item_count: ReorderItems.total_item_count,
//                             total_cart_price: ReorderItems.total_cart_price,
//                             delivery_type: ReorderItems.delivery_type,
//                             is_use_item_tax: ReorderItems.is_use_item_tax,
//                             is_tax_included: ReorderItems.is_tax_included,
//                             store_taxes: ReorderItems.store_taxes,
//                             table_no: ReorderItems.table_no,
//                             no_of_persons: ReorderItems.no_of_persons,
//                             booking_type: ReorderItems.booking_type
//                     });
//                 });
                    ...
                    Order_payment.findOne({ _id: request_data.body.order_payment_id }).then((order_payment) => {
                        card_stripe.pay_stripe_intent_payment(request_data, order_payment, function (payment_paid) {
                            Order_payment.find({ user_id: request_data_body.user_id, cart_id: request_data_body.cart_id, is_payment_paid: false, refund_amount: 0 }).then((order_payments) => {
                                ...
                                {
                                    response_data.json({ success: true, payment: order_payment, user: user });
                                }
                            });
                        });
                                    ...
                                    Cart.findOne({ _id: cart_id }).then((cart) => {
                                        if (cart) {
                                            ...
                                            if (request_data_body.destination_addresses) {
                                                ...
                                                cart.pickup_addresses[0].note=request_data_body.pickup_note
                                                cart.save();
                                            }
...
exports.set_order_status = function (request_data, response_data) {
    ...
    utils.check_unique_details(request_data, [{ name: 'order_id', type: 'string' }, {name: 'order_status', type: 'number'}], function (response) {
        if (response.success) {
            var request_data_body = request_data.body;
                ...
                if (order) {
                    Store.findOne({ _id: order.store_id }).then((store) => {
                        Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                                    ...
                                    exports.capture_payment(store, order, request_data, order_payment, function (capture_payment_response) {
                                        if (capture_payment_response.success) {
                                            exports.store_accept_order(store, order, request_data, order_payment, function (store_accept_response) {
                                                //console.log(store_accept_response)
                            ...
                            } else if (order_status == ORDER_STATE.CUSTOMER_ARRIVED){
                                exports.user_arrived(store, order, request_data, function (store_preparing_response) {
                                    response_data.json(store_preparing_response);
                                })
                            }
        ...
        Payment_gateway.findOne({ _id: order_payment.payment_id }).then((payment_gateway) => {
            if (payment_gateway.name == 'Stripe') {
                ...
                stripe.paymentIntents.capture(
                    ...
                    amount_to_capture: Math.round(order_payment.card_payment * 100),
                ...
                console.log(order_payment.total)
                console.log(order_payment.capture_amount)
                if (order_payment.total < order_payment.capture_amount){
                    console.log('----in if')
                    User.findOne({ _id: order_payment.user_id }).then((user) => {
                        var wallet_amount = order_payment.capture_amount - order_payment.total;
                        var wallet_information = { order_payment_id: order_payment._id };
                        console.log(wallet_amount)
                        var total_wallet_amount = wallet_history.add_wallet_history(ADMIN_DATA_ID.USER, user.unique_id, user._id, user.country_id
                            , user.wallet_currency_code, order_payment.order_currency_code, order_payment.wallet_to_order_current_rate, wallet_amount, user.wallet,
                            WALLET_STATUS_ID.ADD_WALLET_AMOUNT, WALLET_COMMENT_ID.ORDER_CHARGED, "Order Charged: " + order.unique_id, wallet_information);
                        user.wallet = total_wallet_amount;
                        user.save();
...
exports.user_arrived = function (store, order, request_data, response_data){
    utils.check_unique_details(request_data, [{name: 'order_id', type: 'string'}], function (response){
        ...
        User.findOne({ _id: order.user_id }).then((user) => {
            ...
            if (user) {
                ...
                order.order_status = ORDER_STATE.CUSTOMER_ARRIVED;

                var index = order.date_time.findIndex((x) => x.status == ORDER_STATE.CUSTOMER_ARRIVED);
                if (index == -1) {
                    order.date_time.push({ status: ORDER_STATE.CUSTOMER_ARRIVED, date: new Date() });
                } else {
                    order.date_time[index].date = new Date();
                }
...
exports.store_cancel_order = function (store, order, request_data, response_data) {
    //console.log('-------store-cancle-order---------')
...
exports.store_reject_order = function (store, order, request_data, response_data) {
    //console.log('-------store-Reject-order---------')
                                    ...
                                    Request.findOne({ _id: order.request_id }).then((request) => {
                                        ...
                                        Provider.findOne({ _id: current_provider }).then((provider) => {
                                            ...
                                            Store.findOne({ _id: order.store_id }).then((store) => {
                                                ...
                                                order.order_status_manage_id = ORDER_STATUS_ID.CANCELLED;
                                                order.cancel_reason = cancel_reason;
                                                var cancellation_charge_apply_from = 7
                                                var cancellation_charge_apply_till = 5
                                                ...
                                                order.save().then(() => {
                                                    ...
                                                    Order_payment.findOne({ _id: order.order_payment_id }).then((order_payment) => {
                                                        if (order_payment) {
                                                            Payment_gateway.findOne({ _id: order_payment.payment_id }).then((payment_gateway) => {
                                                                if (payment_gateway) {
                                                                    if ((order_status != ORDER_STATE.WAITING_FOR_ACCEPT_STORE && payment_gateway.name == 'Stripe') || payment_gateway.name == 'Paystack') {
                                                                        ////console.log('----------is call------')
                                                                        utils.refund_payment_for_selected_payment_gateway(order_payment, function (response) { });
                                                                    }
                                                                }
                                                                ...
                                                                City.findOne({ _id: order.city_id }).then((city) => {
                                                                    ...
                                                                    //cancel order store selection added
                                                                    ...
                                                                    let query = {
                                                                        $match: { $and: [{ _id: mongoose.Types.ObjectId(request_data_body.order_id) }, { user_id: mongoose.Types.ObjectId(user_id) }] }
                                                                    }
                                                                    Order.aggregate([query, cartLookup, cartUnwind]).then(order_detail => {
                                                                        ////console.log(order_detail)
                                                                        let order_details = order_detail[0]
                                                                        let orderCancellationCharge = 0
                                                                        if (order_details.delivery_type === 3 || is_order_cancellation_charge_apply && order_status >= cancellation_charge_apply_from && order_status <= cancellation_charge_apply_till && order_payment.total_order_price > order_cancellation_charge_for_above_order_price) {
                                                                            ...
                                                                            if (order_details.delivery_type === 3) {
                                                                                ...
                                                                                TableSettings.find({ store_id: mongoose.Types.ObjectId(order_details.store_id) }).then(table_settings => {
                                                                                    if (table_settings) {
                                                                                        ...
                                                                                        if (order_details.cart_details.booking_type === 1 && tableSettingsDetails.is_cancellation_charges_for_without_order) {
                                                                                            ...
                                                                                            if (cancellationChargeType === 1) {
                                                                                                ...
                                                                                                //console.log(orderCancellationCharge)
                                                                                                //console.log(order_details.total)
                                                                                                //console.log(tableCancellationCharges[min_index].value)
                                                                                                //console.log('-------percent-------')
                                                                                            } else {
                                                                                        ...
                                                                                        order_cancellation_charge_value = orderCancellationCharge
                                                                                        order_cancellation_charge_value = utils.precisionRoundTwo(Number(order_cancellation_charge_value));
                                                                                        //console.log('-----------')
                                                                                        //console.log(order_cancellation_charge_value)
